...
public class SingleStdntBill {
    ...
    public Double monthlybill(Map<Integer, BillAmount> billmap, int fromdate, int todate,int hallid){
        ...
        int key=0, bf=0,lunch=0,dinner=0,dateserial=0;
        Date date=null;
        String strdate= "";
        ...
        try{
            ...
            while(rs.next()){
                ...
                try{
                    bfbill = billmap.get(dateserial).bf;
                    lunchbill = billmap.get(dateserial).lunch;
                    dinnerbill = billmap.get(dateserial).dinner;
